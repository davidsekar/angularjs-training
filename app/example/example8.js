...
app.directive('nameCard', function() {
    function link(scope, element, attrs) {
        ...
        element.prepend('<span>' + attrs.num + '</span>');
      }
    return {
        restrict: 'AECM',
        replace: true,
        ...
        // '<hr /></section>',
        template: '<section><div><em>Name :</em> {{customerInfo.name}}</div>'+
        '<div><em>Age :</em> {{customerInfo.age}}</div>' +
        '<div><em>Gender :</em> {{customerInfo.gender}}</div>' +
        '<hr /></section>',
        scope: {
            customerInfo: '=info',
            // customer: '='
